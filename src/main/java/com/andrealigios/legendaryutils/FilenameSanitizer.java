...
public class FilenameSanitizer {
	...
	private static String sanitization(String input, boolean safely, boolean prettily) {
		...
		String output = handleRservedNames(handleReservedChars(filename));
	 ...
	 */
	private static String handleRservedNames(String filename) {
	...
	private static String handleReservedChars(String filename) {
		return filename.codePoints().map(i -> i > 31 ? i : SANITIZATION_TOKEN)
